...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="简单总结起来-流程大概如下。" tabindex="-1"><a class="header-anchor" href="#简单总结起来-流程大概如下。" aria-hidden="true">#</a> 简单总结起来，流程大概如下。</h4>
<ul>
<li>首先，<strong>webpack</strong>会读取项目中由开发者定义的 <strong>webpack.config.js</strong>配置文件，或者从<strong>shell</strong>语句中获得必要的参数。这是<strong>webpack</strong>内部接收业务配置信息的方式。这样就完成了配置读取的譾氛步工作。</li>
<li>接着，将所需的<strong>webpack</strong>插件实例化，在<strong>webpack</strong>事件流上挂载插件钩子,这样在合适的构建过程中，插件就具备了改动产出结果的能力。</li>
<li>同时，根据配置所定义的人口文件，从入口文件(可以不止一个)开始，进行依赖收集，对所有依赖的文件进行编译,这个编译过程依赖<strong>loaders</strong>,不同类型的文件根据开发者定义的不同<strong>loader</strong> 进行解析。编译好的内容使用 <strong>acorn</strong> 或其他抽象语法树能力,解析生成抽象语法树,分析文件依赖关系，将不同模块化语法(如<strong>require</strong>)等替换为**<em>_webpack_require</em><strong>，即使用</strong>webpack**自己的加载器进行模块化实现。</li>
<li>上述步骤完戚后，产出结果，根据开发者配置，将结果打包到相应目录。</li>
</ul>
<p>在整个打包过程中，<strong>webpack</strong> 是一个<strong>基于事件驱动的编译器系统</strong>，其核心架构围绕 <strong>Tapable</strong> 事件流系统构建, <strong>webpack</strong> 和插件都采用基于事件流的发布/订阅模式，某些关键过程，并在这些环节中执行插件任务。最后，所有文件的编译和转化都已经完成，输出劣终资源。
...
<ol>
<li><strong>Compiler</strong> 是 webpack 的核心对象，负责全局控制和流程管理。</li>
<li><strong>Compilation</strong> 是每次构建过程中生成的对象，负责代码解析、模块构建和资源生成。</li>
<li><strong>Compiler</strong> 通过事件钩子（如 <code v-pre>compilation</code>）来触发 <code v-pre>Compilation</code> 的生成和执行。</li>
<li><strong>Compilation</strong> 在构建过程中会经历多个阶段，如模块解析、优化和资源生成。</li>
</ol>
<h4 id="简化版流程图" tabindex="-1"><a class="header-anchor" href="#简化版流程图" aria-hidden="true">#</a> 简化版流程图：</h4>
<p><img src="/img/c-c.svg" alt=""></p>
<div class="language-mermaid line-numbers-mode" data-ext="mermaid"><pre v-pre class="language-mermaid"><code>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="流程解释" tabindex="-1"><a class="header-anchor" href="#流程解释" aria-hidden="true">#</a> 流程解释：</h4>
<ol>
<li><strong>读取配置</strong>：<code v-pre>webpack</code> 首先读取配置文件，初始化构建环境。</li>
<li><strong>webpack 初始化</strong>：<code v-pre>webpack</code> 完成初始化，准备开始构建过程。</li>
<li><strong>Compiler 全局控制</strong>：<code v-pre>Compiler</code> 对象接管全局控制，负责触发构建过程中的各个事件钩子。</li>
<li><strong>Compilation</strong>：<code v-pre>Compiler</code> 触发 <code v-pre>compilation</code> 钩子，生成 <code v-pre>Compilation</code> 对象。</li>
<li><strong>构建模块</strong>：<code v-pre>Compilation</code> 开始解析模块，构建依赖图。</li>
<li><strong>优化资源</strong>：<code v-pre>Compilation</code> 对构建的资源进行优化，如代码压缩、Tree Shaking 等。</li>
<li><strong>产出结果</strong>：<code v-pre>Compilation</code> 生成最终的资源文件，输出到指定目录。</li>
</ol>
<p><code v-pre>webpack</code> 的构建过程是通过 <code v-pre>compiler</code> 控制流程，通过<code v-pre>compilation</code> 进行代码解析的。在开发播件时，我们可以从<code v-pre>compiler</code>对象中得到所有与 webpack 主环境相关的内容，包括事件钩子。更多信息将在下一节介绍。
瀠狛戢 se
<code v-pre>compiler</code> 对象和 <code v-pre>compilation</code>对象都继承自 tapable 库,该库暴露了所有和事件相关的发布/订阅的方法。<code v-pre>webpack</code>中基于事件流的<code v-pre>tapable</code>库不仅能保证插件的有序性，还能使整个系统扩展性更好</p>
</div></template>
